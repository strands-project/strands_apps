...
class ReconfigureInflationServer(object):
    ...
    def execute(self, goal):    
        ...
        local_params = { 'inflation_radius' : 1.8 }
        global_params = { 'inflation_radius' : 1.8 }
        local_config = self.local_reconfig_client.update_configuration(local_params)
        global_config = self.local_reconfig_client.update_configuration(global_params)
        ...
        if status == GoalStatus.ABORTED:
            self.server.set_aborted(self.move_base_action_client.status())
        if status == GoalStatus.PREEMPTED:
            self.server.set_preempted(self.move_base_action_client.status())
        self.server.set_succeded(self.move_base_action_client.status())
